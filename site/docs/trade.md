...
**Boons are capped at five.** You can hold **five** faction boons at once. Only *timed* rewards occupy a slot — an instant lump of resources is unpacked on arrival and holds nothing. While all five slots are full, further encounters return empty-handed (*"Your stores are already thick with foreign gifts; the envoys are thanked, fed, and sent home with their crates unopened"*) until one expires. Each timed boon runs a few hundred ticks, so slots free up steadily and a busy explorer spends only about a third of the time at a full court — the cap is there to give the reward loop a shape, not to punish you for exploring.
...
Encounters are one more reason to keep expeditions running even after you've met everyone — with the caveat that a failed run now brings back a bill, and a run launched with all five boon slots full mostly brings back nothing. See [Military & Expeditions](military.md#faction-encounters).
