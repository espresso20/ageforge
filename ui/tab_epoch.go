...
// EpochTab displays the current epoch status, history, legacy bonuses, and civilization log.
type EpochTab struct {
	root *tview.TextView
}

// NewEpochTab creates the epoch tab.
func NewEpochTab() *EpochTab {
	t := &EpochTab{}
	t.root = tview.NewTextView().
		SetDynamicColors(true).
		SetScrollable(true)
	t.root.SetBorder(true).SetTitle(" Epoch ").SetTitleColor(ColorTitle)
	return t
}

// Primitive returns the root primitive for page registration.
func (t *EpochTab) Primitive() tview.Primitive {
	return t.root
}

// Refresh updates the epoch tab with current game state.
func (t *EpochTab) Refresh(state game.GameState) {
	var sb strings.Builder

	t.renderCurrentEpoch(&sb, state)
	sb.WriteString("\n")
	t.renderEpochHistory(&sb, state)
	sb.WriteString("\n")
	t.renderLegacyBonuses(&sb, state)
	sb.WriteString("\n")
	t.renderCivilizationLog(&sb, state)

	t.root.SetText(sb.String())
}

// renderCurrentEpoch renders Section 1: current epoch status.
func (t *EpochTab) renderCurrentEpoch(sb *strings.Builder, state game.GameState) {
	if state.EpochKey == "" {
	...
	if !ok {
		...
		for _, a := range ep.Ages {
			// Format age key to display name: replace underscores, title-case each word
			ageNames = append(ageNames, formatAgeKey(a))
		}
	...
	if currentEvent == nil {
		...
		if currentEvent.EventType == "catastrophe" {
			...
			// Look up flavor text and duration from config
			...
			if evDef, found := evDefs[currentEvent.EventKey]; found {
				...
				if evDef.Duration > 0 {
					// Duration remaining can't be computed here (no tick diff), show static
...
// renderEpochHistory renders Section 2: epoch history across all epochs.
func (t *EpochTab) renderEpochHistory(sb *strings.Builder, state game.GameState) {
	...
	for _, ep := range allEpochs {
		if ep.Order > currentEpochOrder {
			break // don't show future epochs
		}

		isCurrent := ep.Key == state.EpochKey

		// Find event record for this epoch
...
// renderLegacyBonuses renders Section 3: legacy bonuses from Succumb.
func (t *EpochTab) renderLegacyBonuses(sb *strings.Builder, state game.GameState) {
	...
	// Iterate epochs in order so display is stable
	allEpochs := config.Epochs()
	anyShown := false
	for _, ep := range allEpochs {
		if !state.LegacyBonuses[ep.Key] {
...
// renderCivilizationLog renders Section 4: civilization catastrophe log.
func (t *EpochTab) renderCivilizationLog(sb *strings.Builder, state game.GameState) {
...
// formatAgeKey converts a snake_case age key into a display-friendly title.
// e.g. "colonial_age" -> "Colonial Age"
func formatAgeKey(key string) string {
