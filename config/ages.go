...
type AgeDef struct {
	...
	Description     string
	// Quip is a short, in-voice one-liner shown beneath the formal Description on
	// the age-advance splash. Cosmetic only; tone scales from caveman observation
	// (early ages) to existential dread (late ages). Should never be empty.
	Quip     string
...
func Ages() []AgeDef {
	return normalizeAgeRequirements([]AgeDef{
		// === 0: PRIMITIVE AGE (Stone Era) ===
		{
			...
			Quip:            "Fire: discovered. Fingers: regrettably also discovered.",
